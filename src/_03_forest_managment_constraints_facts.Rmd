...
haz_fuel <- sf::st_read(
  ...
  haz_fuel <- haz_fuel |>  
    ...
    dplyr::rename(treatment_category = cat_nm) |> 
    dplyr::mutate(
      facts_src = "haz_fuel"
      , treatment_type = trimws(treatment_type)
      # , treatment_area_ha = as.numeric(sf::st_area(geometry))/10000
    ) |> 
    dplyr::select(suid, facts_src, treatment_category, treatment_type, date_completed)
  gc()
# timber harvest
harvests <- sf::st_read(
...
Combine treatment layers with areas of interest
...
treatment_in_landscapes <- rbind(
  ...
  dplyr::summarise(
    ...
    , dplyr::across(
      c(tidyselect::starts_with("pct_"), tidyselect::ends_with("_ha"))
      , ~ mean(.x,na.rm=T)
    )
  ) |> 
  dplyr::mutate(
