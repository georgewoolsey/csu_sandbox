...
ggplot(
  ...
  theme(
    ...
    , axis.text.x = element_text(size=7) # element_blank() #
...
plt_fn_temp <- function(scnum=1) {
  ...
  labs(
    fill = ""
    , tag = map_fs_temp |> dplyr::filter(scenario_id==scnum) |> 
...
scenario_cnstrnt_map_fn <- function(row_n=1) {
  ...
  zoom_level <- ifelse(
    (wf_landscapes |> 
      ...
      purrr::pluck(1)
      <= if_smaller_ha_temp
    & area_nm_list[row_n]!="UT: Wasatch"
    ) 
    # | area_nm_list[row_n] %in% c("WA: Colville Northeast Washington Vision")
    , 10
    , 8
  )
...
plt <- ggplot(
  ...
  # scale_x_continuous(expand = expansion(mult = c(0, .15)),labels = scales::percent_format()) +
  scale_x_continuous(limits = c(0, .80),labels = scales::percent_format()) +
  labs(
    fill = "Level of\nConstraint"
    , y = ""
    , x = ""
  ...
  theme(
    ...
    , strip.background = element_rect(fill = "gray88")
    # , strip.placement = "outside"
    , plot.margin = unit(c(-.15,0,-4,0),"cm")
    , plot.subtitle = element_text(vjust = -1,face="bold")
  ) +
  guides(
    fill = guide_legend(reverse = T, override.aes = list(alpha = 0.9))
  )
