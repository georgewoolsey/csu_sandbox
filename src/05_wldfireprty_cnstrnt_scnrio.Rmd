...
constrained_by_scnro_ls <- list.files("../data/wildfirepriority/all_rd/",pattern = "\\.csv$") %>%
  purrr::keep(stringr::str_starts(.,"wfpriority_all_rd_sc")) %>% 
  purrr::map(function(x){
    readr::read_csv(
      paste0("../data/wildfirepriority/all_rd/",x)
...
constrained_by_scnro_ls_pa <-
  list.files("../data/wildfirepriority/fireshed_rd/",pattern = "\\.csv$") %>%
  purrr::keep(stringr::str_starts(.,"wfpriority_fireshed_rd_sc")) %>% 
  purrr::map(function(x){
    readr::read_csv(
      paste0("../data/wildfirepriority/fireshed_rd/",x)
      , name_repair = "universal"
      , col_types = cols(.default = "c")
    ) %>% 
    dplyr::rename_with(tolower) %>% 
...
*Nested spatial framework for firesheds. Each scale has specific functionality in terms of the planning processes. Firesheds are the broad scale unit of prioritization, but planning areas within them are also prioritized as part of implementation of treatments. Planning areas are roughly the size that national forests use for conducting vegetation and fuel management projects. The relative variation among firesheds compared to variation within them controls the relative emphasis on selecting firesheds versus individual planning areas.* [Ager et al. 2021](https://scholar.google.com/scholar?cluster=7852635540589253195&hl=en&as_sdt=0,6)
...
plt_basemap + 
  ...
  theme(
      legend.position = c(0.85,0.85) # "top"
      # , legend.direction = "horizontal"
      , legend.title = element_text(size = 8)
      , legend.text = element_text(size = 7)
    )
...
plt_inset <-
  plt_fshed +
    ggrepel::geom_text_repel(
      data = wf_landscapes |> dplyr::filter(state %in% c("Colorado","New Mexico"))
      , aes(
          label = stringr::str_wrap(
            dplyr::case_when(
              stringr::str_count(name, "\\w+")<2
...
### Fireshed Planning Areas >25% w/in Landscape
...
*Only including fireshed planning areas that have at least 25% of their area within the bounds of the priority landscapes and are in a high-risk fireshed.*
...
*Using the area of fireshed planning areas that have at least 25% of their area within the bounds of the priority landscapes and are in a high-risk fireshed as the treatment area objective (20-40%).*
...
kableExtra::kable(
  ...
  , caption = "Wildfire Crisis Strategy High-Risk Fireshed Planning Areas<br>mechanically available for treatment in lightly and moderately constrained planning areas (≥25% in landscape)"
  , col.names = c(
    ""
    , "Total\nLandscape"
    , "High-risk\nFireshed PAs"
  ...
  , escape = F
    ...
    add_header_above(c(" " = 1, "Area (ha)" = 3, "Treatable High-Risk Area (ha)"=3, "Percent of High-Risk Area" = 3)) |>
    kable_classic(full_width=T) |> 
...
What is the landcover classification of the fireshed planning area units? This [GEE script](https://code.earthengine.google.com/578bb6ba1f5c534bfd9f1b6a0c7d0541?noload=true) was used to calculate the area by NLCD landcover class.
...
# load nlcd cover data by fireshed planning area
nlcd_area_ls_pa <- 
...
The plot above shows the NLCD landcover class distribution for *all* fireshed planning areas with at least *25%* of area within the boundary of the Strategy priority landscape.
...
The plot above shows the NLCD landcover class distribution for only *high-risk* fireshed planning areas with at least *25%* of area within the boundary of the Strategy priority landscape.
...
ggplot(
  ...
  facet_grid(cols = vars(class_label), rows = vars(scenario_lab)) +
  scale_x_continuous(labels = scales::percent_format()) +
...
In the plot above fireshed planning areas are grouped into forest or shrubland based on the majority cover type (e.g. >50% forest). Fireshed planning areas where the majority covertype is not forest or shrubland are not shown.

```{r nlcd-trt-cnstrnt}
nlcd_area_ls_pa_cl_temp |> 
  dplyr::filter(pct_area > .5) |> 
  dplyr::count(class_label, scenario_lab, cnstrnt_class) |> 
  dplyr::group_by(class_label, scenario_lab) |> 
  dplyr::mutate(pct = n/sum(n)) |> 
ggplot() +
  geom_col(
    mapping = aes(x = pct, y = class_label, fill=cnstrnt_class)
    ,width = 0.7, alpha=0.8
  ) +
  geom_text(
    mapping = aes(x = pct, y = class_label, group=cnstrnt_class
        ,label = scales::percent(ifelse(pct>=0.05,pct,NA), accuracy = 1)
      )
    , position = position_stack(vjust = 0.5)
    , color = "black", size = 3
