# Wildfire Crisis Strategy Priority Landscapes{#wcs_intro}
...
The USFS recently launched a [Wildfire Crisis Strategy](https://www.fs.usda.gov/managing-land/wildfire-crisis) (WCS) plan to substantially increase the scale of forest health and risk reduction fuel treatments over the next decade. The plan prioritizes treatments on 21 landscapes in eight states in the western US targeting federal, state, tribal, and private lands where wildfire ignitions will potentially impact communities.
...
## Load landscape-level data{#wflndscp}
...
# join with spatial data and write out
constrained_by_wflndscp_wide_sf <- wf_landscapes |> 
  dplyr::select(state,name,investment,tidyselect::ends_with("res")) |> 
  dplyr::left_join(
    constrained_wide
    , by = dplyr::join_by(state,name)
  )
# save for reading later
sf::st_write(constrained_by_wflndscp_wide_sf, "../data/constrained_by_wflndscp_wide_sf.gpkg", append=F)
```
...
Note, some of these areas have a very low percentage of forested area. For example, only **`r constrained_wide |> dplyr::arrange(pct_forested_area) |> dplyr::filter(dplyr::row_number()==1) |>  dplyr::pull(pct_forested_area) |> scales::percent(accuracy=1)`** of the **`r constrained_wide |> dplyr::arrange(pct_forested_area) |> dplyr::filter(dplyr::row_number()==1) |>  dplyr::pull(area_name)`** landscape is forested (NLCD class 41-43).

What is the landcover classification of the remaining area? This [GEE script](https://code.earthengine.google.com/69cee569d87fa6ce27723fcf579fbb32?noload=true) was used to calculate the area by NLCD landcover class.
...
wf_nlcd_area |> 
  ...
  dplyr::ungroup() |> 
  dplyr::left_join(
    constrained_wide |> dplyr::select(area_name,pct_forested_area)
    , by = dplyr::join_by(area_name)
  ) |> 
ggplot(
    mapping = aes(x = pct_area, y = reorder(area_name, -pct_forested_area), fill=class_label)
  ) +
...
What should be done about these low-forest landscapes? Include shrubland as part of area considered for constraint analysis.
...
Lightly and Moderately Constrained

```{r lt-mod-cnstrnt-tab, results='asis'}
## area of low sws
xx_temp <- constrained_byftr_huc12_wide |> 
  dplyr::group_by(state,name, cnstrnt_class) |> 
  dplyr::summarise(
    dplyr::across(
      dplyr::ends_with("area_ha")
      , sum
    )
  ) |> 
  ...
  kableExtra::kable(
    xx_temp |> dplyr::select(-c(state))
    , caption = "Subwatershed forested area of each Wildfire Crisis Strategy Priority Landscape<br>available for treatment in lightly and moderately constrained subwatersheds"
    , col.names = c(
      ""
  ...
  add_header_above(c(" " = 1, "Treatable Forested Area (ha)"=3, "Percent of Total\nSubwatershed Forested Area" = 3)) |>
  kable_classic(full_width=T) |> 
  pack_rows(index = table(forcats::fct_inorder(xx_temp$state))) |>
  kableExtra::kable_styling(font_size = 11,fixed_thead = TRUE) |>  
  kableExtra::scroll_box(width = "740px")
...
### Map of Subwatersheds by Level of Constraint{#wf_read_sw}
...
constrained_byftr_huc12_wide_sf <- constrained_byftr_huc12_wide |> 
  ...
  sf::st_as_sf() 
# save for reading later
sf::st_write(constrained_byftr_huc12_wide_sf, "../data/constrained_byftr_huc12_wide_sf.gpkg", append=F)
# polish for mapping
constrained_byftr_huc12_wide_sf <- constrained_byftr_huc12_wide_sf |> 
  dplyr::mutate(
    dplyr::across(
      tidyselect::starts_with("pct_")
...
# ggplot map
plt_temp <- ggplot() + 
  geom_sf(data = constrained_byftr_huc12_wide_sf, aes(fill = cnstrnt_class), lwd = 0.05) +
  geom_sf(
    data = USAboundaries::us_states(states = c(
      ...
      sf::st_transform(sf::st_crs(constrained_byftr_huc12_wide_sf))
    , fill = NA
    , color = "gray55"
  ) +
...
plt_temp
ggsave("../data/map_sws2.png", plt_temp, height = 6, width = 5, dpi = 500)
...
split_poly_fn <- function(sf_poly, n_areas){
  ...
  voronoi_polys <- dismo::voronoi(k_means$centers, ext = sf_poly |> sf::as_Spatial()) |> 
    sf::st_as_sf()
  # clip to sf_poly
  sf::st_crs(voronoi_polys) <- sf::st_crs(sf_poly)
...
# write
fire_landscapes_split |> 
  # dplyr::filter(NAME == "Klamath River Basin") |> 
