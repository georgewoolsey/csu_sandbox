...
[Preliminary example here](https://georgewoolsey.github.io/usfs_sandbox/forest-management-constraints-versus-activity.html#map-of-constraints-and-management)
...
constrained_byftr_huc12_wide <- 
  list.files("../data/subwatersheds/",pattern = "\\.csv$") |> 
    purrr::map(function(x){
      readr::read_csv(
        paste0("../data/subwatersheds/",x)
...
# covertype_area_ha
constrained_byftr_huc12_wide |> 
  dplyr::filter(region=="Region 2") |> 
  nrow()
# covertype_area_ha
constrained_byftr_huc12_wide |> 
  dplyr::filter(region=="Region 2") |> 
  # dplyr::select(ends_with("_ha")) |> 
  dplyr::pull(covertype_area_ha) |> 
  sum()
# rmn5_roads_area_ha
constrained_byftr_huc12_wide |> 
  dplyr::filter(region=="Region 2") |> 
  dplyr::pull(rmn5_roads_area_ha) |> 
  sum()
# usfs
constrained_wide |> 
  dplyr::group_by(region) |> 
...
constrained_wide |> 
  dplyr::filter(region=="Region 2") |> 
  dplyr::group_by(forest_shortname) |> 
  dplyr::summarise(feature_area_ha=sum(feature_area_ha))
