...
<!--chapter:end:01_usfs_cnstrnts.Rmd-->
...
<!--chapter:end:02_wldfireprty_cnstrnts.Rmd-->
...
ggplot(
    ...
    , mapping = aes(y = pct_treated_area, x = pct_rmn5_roads, color = treatment_category)
  ) +
  ...
  ggrepel::geom_text_repel(
    mapping = 
      aes(label = stringr::str_wrap(
        ifelse(
          stringr::str_count(area_name, "\\w+")<4
          , stringr::word(area_name,1,stringr::str_count(area_name, "\\w+"))
          , stringr::word(area_name,1,4)
        )
        , 7)
      )
    , size = 1.5
  ...
  labs(
    ...
    , x = "Forested Area Mechanically Available (%)"
    , title = paste0(
...
ggplot(
  ...
  , mapping = aes(y = cnstrnt_class, pct_treated_area, fill = cnstrnt_class)
) +
  geom_col(width = 0.7) +
  geom_text(
    mapping = aes(
...
```{r}
# treatment_by_type_area |> 
#   dplyr::select(area_name,treatment_category,treatment_area_ha) |> 
#   write.csv("../data/treatment_by_type_area.csv", row.names = F)

# xxx_treatment_by_type_area <- readr::read_csv("../data/treatment_by_type_area.csv")
# ggplot(xxx_treatment_by_type_area,aes(x=treatment_category,y=treatment_area_ha,fill=treatment_category)) +
#   geom_col() +
...
ggplot(
  ...
  geom_text(
    mapping = aes(
      label = scales::percent(ifelse(pct_treatment_area>.03,pct_treatment_area,NA), accuracy = 1)
    )
    , position = position_stack(vjust = 0.5)
    , color = "black", size = 3
  ) +
  facet_grid(cols = vars( treatment_category))+
  scale_fill_manual(values = c("Forest"="forestgreen","Shrubland"="tan2","Other"="gray77")) +
  scale_x_continuous(labels = scales::percent_format()) +
  labs(
    fill = "NLCD\nCover\nType"
    , y = ""
    , x = "Percent of Area Treated"
    , title = paste0(
      "Treatments Completed FY-"
...
<!--chapter:end:03_forest_managment_constraints_facts.Rmd-->
