...
![USFS Region Map.](../data/usfs_region_map.png)
...
![Wildfire Crisis Strategy Priority Landscapes.](../data/wildfire_crisis_img.jpg){width=30%,height=50%}
```{r, warning=FALSE, message=FALSE, echo=FALSE, include=FALSE}
remove(list = ls())
gc()
```
...
constrained_wide_byftr <-
  list.files("../data/wildfirepriority/")[grep("wildfirepriority_statistics",list.files("../data/wildfirepriority/"))] |> 
    purrr::map(function(x){read.csv(paste0("../data/wildfirepriority/",x))}) |> 
    dplyr::bind_rows() |> 
      dplyr::rename_with(tolower) |> 
      dplyr::rename_with(make.names) |> 
      dplyr::rename(feature_id = system.index) |> 
